...
void fillT(std::vector<Real> x, Grid& T) {//add your own parameters
	...
	//make sure that the temperature in boundary cells stays zero
	for (size_t i = 0; i < T.getM(); i++)
	{
		for (size_t j = 0; j < T.getN(); j++)
		{
			T.changeMatrixElement(i, j, x.at(i + j));
		}
	}
}

void setupA(SparseMatrix<Real>& A, double factor, int N) {//add your own parameters
	// to be implemented
